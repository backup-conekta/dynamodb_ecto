defmodule Dynamo.Ecto do
  @moduledoc """
  Ecto integration with DynamoDB.

  This document will present a general overview of using DynamoDB with Ecto,
  including common pitfalls and extra functionalities.

  Check the [Ecto documentation](http://hexdocs.pm/ecto) for an introduction
  or [examples/simple](https://github.com/michalmuskala/dynamodb_ecto/tree/master/examples/simple)
  for a sample application using Ecto and DynamoDB.

  ## Repositories

  The first step to use DynamoDB with Ecto is to define a repository
  with `Dynamo.Ecto` as an adapter. First define a module:
  ...
  With the repository defined, we can define our models:
      ...
      defmodule Weather do
        ...
        # weather is the DynamoDB collection name
        schema "weather" do
          field :city,    :string
  ...
  `:integer` for SQL databases, and is not handled by DynamoDB. You need to
  ...
  The name of the primary key is just a convenience, as DynamoDB forces us to
  ...
  modules. All Ecto types, except `:decimal`, are supported by `Dynamo.Ecto`.
  ...
  `Dynamo.Ecto` also supports writing queries in Elixir to interact with
  your DynamoDB. Let's see an example:
  ...
  There is also support for count function in queries that uses `DynamoDB`'s
  ...
  Please note that not all Ecto queries are valid DynamoDB queries. The adapter
  ...
  To ease of using in more advanced queries, there is `Dynamo.Ecto.Helpers` module
  you could import into modules dealing with queries.
  Please see the documentation of the `Dynamo.Ecto.Helpers` module for more
  ...
  you to use parameters specific to DynamoDB `find` function:
  ...
  DynamoDB has many administrative commands you can use to manage your database.
  We support them thourgh `Dynamo.Ecto.command/2` function.

      Dynamo.Ecto.command(MyRepo, createUser: "ecto", ...)

  We also support one higher level command - `Dynamo.Ecto.truncate/1` that is
  used to clear the database, i.e. during testing.

      Dynamo.Ecto.truncate(MyRepo)
  ...
  Keep in mind that Ecto associations are stored in different Dynamo
  collections and multiple queries may be required for retriving them.

  While `Dynamo.Ecto` supports almost all association features in Ecto,
  keep in mind that DynamoDB does not support joins as used in SQL - it's
  ...
  some queries, that are not supported by DynamoDB as well. One such call
  ...
  parent model, and that fits DynamoDB's design perfectly.
  ...
  Although schema migrations make no sense for databases such as DynamoDB
  there is one field where they can be very benefitial - indexes. Because of
  this Dynamodb.Ecto supports Ecto's database migrations. You can generate a
  migration with:
  ...
  Because DynamoDB does not support (or need) database schemas majority of the
  functionality provided by `Ecto.Migration` is not useful when working with
  DynamoDB. As we've already noted the most useful part is indexing, but there
  ...
  DynamoDB adapter does not support `create_if_not_exists` or `drop_if_exists`
  migration functions.

  ## DynamoDB adapter features

  The adapter uses `dynamodb` for communicating with the database and a pooling
  library such as `poolboy` for managing connections.

  The adapter has support for:

    * documents with ObjectID as their primary key
    * insert, find, update, remove and count dynamo functions
    ...
    * partial updates using `change_map/2` and `change_array/2` from the
      `Dynamo.Ecto.Helpers` module
    * queries using javascript expresssion and regexes using respectively
      `javascript/2` and `regex/2` functions from `Dynamo.Ecto.Helpers` module.

  ### DynamoDB adapter options
  ...
  recompilation in order to make an effect.
    ...
    * `:adapter` - The adapter name, in this case, `Dynamo.Ecto`
    * `:pool` - The connection pool module, defaults to `Dynamo.Pool.Poolboy`
  ...
  def start_link(repo, opts) do
    {:ok, _} = Application.ensure_all_started(:dynamodb_ecto)

    repo.__dynamo_pool__.start_link(opts)
  end
  ...
  def stop(pid, timeout) do
    ...
    Application.stop(:dynamodb_ecto)
    :ok
  end
  ...
  def insert(repo, meta, params, nil, [], opts) do
    ...
    case Connection.insert(repo.__dynamo_pool__, normalized, opts) do
      {:ok, _} ->
        {:ok, []}
      other ->
        other
  ...
  def update(repo, meta, fields, filter, {pk, :binary_id, _value}, [], opts) do
    ...
    Connection.update(repo.__dynamo_pool__, normalized, opts)
  end
  ...
  def delete(repo, meta, filter, {pk, :binary_id, _value}, opts) do
    ...
    Connection.delete(repo.__dynamo_pool__, normalized, opts)
  end
  ...
  def execute_ddl(_repo, string, _opts) when is_binary(string) do
    raise ArgumentError, "DynamoDB adapter does not support SQL statements in `execute`"
  end
  ...
  def execute_ddl(_repo, {:create_if_not_exists, %Table{}, _columns}, _opts) do
    raise ArgumentError, "DynamoDB adapter supports options for collection only in the `create` function"
  end

  def execute_ddl(_repo, {:create_if_not_exists, %Index{}}, _opts) do
    raise ArgumentError, "DynamoDB adapter does not support `create_if_not_exists` for indexes"
  end

  def execute_ddl(_repo, {:drop_if_exists, _}, _opts) do
    raise ArgumentError, "DynamoDB adapter does not support `drop_if_exists`"
  end
