defmodule Dynamo.Ecto.NormalizedQuery do
  @moduledoc false
  ...
  defp primary_key(model) do
    case model.__schema__(:primary_key) do
      ...
      [pk] -> pk
      keys ->
        raise ArgumentError, "DynamoDB adapter does not support multiple primary keys " <>
  ...
  defp check_query!(query, allow \\ []) do
    ...
    |> Enum.each(fn {element, empty} ->
      check(Map.get(query, element), empty, query,
            "DynamoDB adapter does not support #{element} clause in this query")
    end)
  end
  ...
  Enum.map(binary_ops, fn {op, dynamo_op} ->
    defp binary_op(unquote(op)), do: unquote(dynamo_op)
  end)

  Enum.map(bool_ops, fn {op, dynamo_op} ->
    defp bool_op(unquote(op)), do: unquote(dynamo_op)
  end)
  ...
  defp error(query, place) do
    raise Ecto.QueryError, query: query,
      message: "Invalid expression for DynamoDB adapter in #{place}"
  end
  defp error(place) do
    raise ArgumentError, "Invalid expression for DynamoDB adapter in #{place}"
  end
end
