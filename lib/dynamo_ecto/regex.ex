defmodule Dynamo.Ecto.Regex do
  @moduledoc """
  An Ecto type to represent DynamoDB's Regex objects

  Both DynamoDB and Elixir use PCRE (Pearl Compatible Regular Expressions),
  so the syntax is similar for both.

  ## Using in queries

  DynamoDB supports two ways to use regexes.

  One is to use regex object:

      regex = Dynamo.Ecto.Helpers.regex("elixir", "i")
  ...
  refer to the DynamoDB documentation:
  http://docs.dynamodb.org/manual/reference/operator/query/regex/#syntax-restrictions
  ...
  @doc """
  Converts a `Dynamo.Ecto.Regex` into `BSON.Regex`
  """
  ...
  @doc """
  Converts a `BSON.Regex` into `Dynamo.Ecto.Regex`
  """
